...
require('dotenv').config({ path: 'C:/Users/admin/Documents/TA project/.env.local' });
...
async function main() {
  ...
  const don = await supabase.from('donations').insert({
    ...
    pickup_window_start: new Date(Date.now()).toISOString(),
    pickup_window_end: new Date(Date.now() + 7200000).toISOString(),
    status: 'pickup_scheduled'
  }).select().single();
  
  if (don.error) {
    console.error("Donation insert error:", don.error);
    process.exit(1);
  }
...
setInterval(async () => {
  ...
  console.log("Inserted location step", steps);
  steps++;
  if (steps > 15) process.exit(0);
}, 4000);
  `;
  fs.writeFileSync('C:/Users/admin/.gemini/antigravity-ide/brain/1850dfec-8c90-48bb-9af4-50007cb932cf/scratch/bg_location_simulator.js', bgScript);
}

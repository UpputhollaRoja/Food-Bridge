...
interface DonorLocationPickerProps {
  /** UUID of the donation row to update */
  donationId: string
  /** Optional callback after a successful Supabase save */
  onSaved?: (location: SavedLocation) => void
}

// ─── Status union ────────────────────────────────────────────────
type Status = 'idle' | 'locating' | 'saving' | 'saved' | 'error'
...
// ════════════════════════════════════════════════════════════════
export default function DonorLocationPicker({
  donationId,
  onSaved,
}: DonorLocationPickerProps) {
  ...
  const saveToSupabase = useCallback(
    async (loc: SavedLocation) => {
      ...
      const { error } = await supabase
        .from('donations')
        .update({
          pickup_location: JSON.stringify(loc),
        })
        .eq('id', donationId)
